---
name: mega
description: Route any MEGA workflow between the official 39-tool MEGA MCP v1.3 server and the mega CLI. Use for MEGA identity, models, datasets, repositories, Spaces, papers, documentation, community, Jobs, Sandboxes, account memory, inference, uploads, downloads, Git operations, or when choosing the safest execution surface.
license: MIT
---
...
| Durable account preferences, context, or standing instructions | MCP Memory tools with opt-in scopes |
...
- Account memory: `memory_list`, `memory_search`, `memory_remember`, `memory_update`, `memory_forget`, `memory_set_enabled`, `memory_clear` with tool-level OAuth.
...
Use the narrower domain skill for detailed procedures: `$mega-repositories`, `$mega-datasets`, `$mega-spaces`, `$mega-jobs`, `$mega-inference`, `$mega-community`, `$mega-memory`, or `$mega-cli`.
